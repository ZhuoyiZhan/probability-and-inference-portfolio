...
To win in World Series, the Braves can win in 4th, 5th, 6th, 7th game because it needs to win 3 out of the first (N-1) games and the last game.
...
dnbinom(x,y,p) measures probability of x failure before yth success given that probability of success is p.

```{r}
sum(dnbinom(0:3,4,0.55))#the total probability of winning in 4th, 5th, 6th, and 7th game.
```

Given that Pb=0.55, the Brave has a 0.608 probability of winning the World Series.
...
This is a figure of Pb vs P(the Braves Win World Series). There is a positive relationship between pb and the Braves win the World Series. When Pb increases, the probability that the brave win World Series increases.
...
When Pb = 0.55, the shortest series length is 71 when P(Braves win World Series|PB=.55) is 0.802.
...
There is a negative relationship between Pb and shortest series length. As Pb increase, the shortest series length decreases. 
...
The P(Pb =0.55|Braves winworld series) is 0.55.
